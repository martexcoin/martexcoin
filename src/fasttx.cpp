...
void ProcessMessageFastTX(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (!sporkManager.IsSporkActive(SPORK_2_FASTSEND_ENABLED)) return;
...
bool IsIXTXValid(const CTransaction& txCollateral)
{
    ...
    if (nValueOut > sporkManager.GetSporkValue(SPORK_5_FASTSEND_MAX_VALUE) * COIN) {
        LogPrint("fasttx", "%s : Transaction value too high - %s\n", __func__, txCollateral.ToString().c_str());
        return false;
    }

    if (missingTx) {
        LogPrint("fasttx", "%s : Unknown inputs in IX transaction - %s\n", __func__, txCollateral.ToString().c_str());
        /*
    ...
    if (nValueIn - nValueOut < COIN * 0.01) {
        LogPrint("fasttx", "%s : did not include enough fees in transaction %d\n%s\n", __func__, nValueOut - nValueIn, txCollateral.ToString().c_str());
        return false;
    }
...
int64_t CreateNewLock(CTransaction tx)
{
    ...
    */
    ...
    if (!mapTxLocks.count(tx.GetHash())) {
        ...
        LogPrint("fasttx", "%s : Transaction Lock Exists %s !\n", __func__, tx.GetHash().ToString().c_str());
    }
...
void DoConsensusVote(CTransaction& tx, int64_t nBlockHeight)
{
    ...
    if (n == -1) {
        LogPrint("fasttx", "%s : Unknown Masternode\n", __func__);
        return;
    }

    if (n > FASTTX_SIGNATURES_TOTAL) {
        LogPrint("fasttx", "%s : Masternode not in the top %d (%d)\n", __func__, FASTTX_SIGNATURES_TOTAL, n);
        return;
    }
    /*
    ...
    */
    ...
    LogPrint("fasttx", "%s : In the top %d (%d)\n", __func__, FASTTX_SIGNATURES_TOTAL, n);
...
bool ProcessConsensusVote(CNode* pnode, CConsensusVote& ctx)
{
    ...
    if (pmn != NULL)
        LogPrint("fasttx", "%s : Masternode ADDR %s %d\n", __func__, pmn->addr.ToString().c_str(), n);
    ...
    if (n > FASTTX_SIGNATURES_TOTAL) {
        LogPrint("fasttx", "%s : Masternode not in the top %d (%d) - %s\n", __func__,
...
int GetTransactionLockSignatures(uint256 txHash)
{
    ...
    if (!sporkManager.IsSporkActive(SPORK_2_FASTSEND_ENABLED)) return -1;
...
bool CTransactionLock::SignaturesValid()
{
    for (CConsensusVote vote : vecConsensusVotes) {
        ...
        if (n > FASTTX_SIGNATURES_TOTAL) {
            return error("%s : Masternode not in the top %s", __func__, FASTTX_SIGNATURES_TOTAL);
        }
