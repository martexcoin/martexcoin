...
//
// Bootup the Masternode, look for a 5000 MXT input and register on the network
//
...
std::vector<COutput> CActiveMasternode::SelectCoinsMasternode()
{
    ...
    for (const COutput& out : vCoins) {
        if (out.tx->vout[out.i].nValue == Params().GetConsensus().nMasternodeCollateral * COIN) { //exactly
