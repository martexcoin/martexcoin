    ...
    walletModel(model)
{
    ...
    ui->lineEditPort->setPlaceholderText("e.g 51315");
    ...
    if (walletModel->isRegTestNetwork()) {
        ui->lineEditPort->setText("31315");
    } else if (walletModel->isTestNetwork()) {
        ui->lineEditPort->setText("41315");
    } else {
        ui->lineEditPort->setText("51315");
    }
...
bool MasterNodeWizardDialog::createMN()
{
    ...
    if (!walletModel->getMNCollateralCandidate(collateralOut)) {
        ...
        SendCoinsRecipient sendCoinsRecipient(QString::fromStdString(address.ToString()), QString::fromStdString(alias), CAmount(Params().GetConsensus().nMasternodeCollateral) * COIN, "");
        ...
        for (int i=0; i < (int)walletTx->vout.size(); i++) {
            ...
            if (out.nValue == Params().GetConsensus().nMasternodeCollateral * COIN) {
                indexOut = i;
                break;
            }
        }
