...
ColdStakingWidget::ColdStakingWidget(MARTEXGUI* parent) :
    PWidget(parent),
    ...
    isLoading(false)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can delegate your MXT, letting a hot node (24/7 online node)\nstake on your behalf, while you keep the keys securely offline."));
    ...
    ui->labelSubtitle2->setText(tr("Accept MXT delegation / Delegate MXT"));
    ...
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select MXT outputs to delegate.");
...
void ColdStakingWidget::onCoinControlClicked(){
    if(isInDelegation) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any MXT to select."));
        }
    }
}
...
void ColdStakingWidget::updateStakingTotalLabel()
{
    ...
    ui->labelStakingTotal->setText(tr("Total Staking: %1").arg(
            (total == 0) ? "0.00 MXT" : GUIUtil::formatBalance(total, nDisplayUnit))
    );
}
