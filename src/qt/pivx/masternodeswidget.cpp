...
MasterNodesWidget::MasterNodesWidget(MARTEXGUI *parent) :
    PWidget(parent),
    ...
    if (dialog->exportMN) {
        if (ask(tr("Remote Masternode Data"),
                tr("You are just about to export the required data to run a Masternode\non a remote server to your clipboard.\n\n\n"
                   "You will only have to paste the data in the martex.conf file\nof your remote server and start it, "
...
void MasterNodesWidget::onCreateMNClicked(){
    ...
    if (walletModel->getBalance() <= (COIN * Params().GetConsensus().nMasternodeCollateral)) {
        inform(tr("Not enough balance to create a masternode, 5,000 MXT required."));
        return;
    }
    showHideOp(true);
