...
CNode* ConnectNode(CAddress addrConnect, const char* pszDest, bool anonSendMaster, bool fCountFailure)
{
    if (pszDest == NULL) {
...
void CheckOffsetDisconnectedPeers(const CNetAddr& ip)
{
    ...
    if (setOffsetDisconnectedPeers.size() >= MAX_TIMEOFFSET_DISCONNECTIONS) {
        ...
        std::string strWarn2 = _("Please check that your computer's date and time are correct! If your clock is wrong MarteX Core will not work properly.");
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        std::string strDesc = "MARTEX " + FormatFullVersion();
...
void ThreadOpenConnections()
{
    ...
    while (true) {
        ...
        while (true) {
            ...
            if (addr.GetPort() != Params().GetDefaultPort() && !Params().AllowMultiplePorts() && nTries < 50)
                continue;
