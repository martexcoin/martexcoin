...
/*
    ...
    if (GetTime() > START_FOUNDATION_PAYMENTS_TESTNET && Params().NetworkID() == CBaseChainParams::TESTNET){
      nSubsidy = nBlockPoWReward_MMF;
    }else if (GetTime() > START_FOUNDATION_PAYMENTS && Params().NetworkID() == CBaseChainParams::MAIN){
      nSubsidy = nBlockPoWReward_MMF;
    }
*/
    if (nHeight + 1 > nReservePhaseStart && nHeight + 1 < nReservePhaseEnd) {
      nSubsidy = nBlockRewardReserve;
    }

    // Time Swap old version
    if (nHeight + 1 > nReservePhaseEnd && nHeight + 1 < 10080 && Params().NetworkID() == CBaseChainParams::MAIN) {
       nSubsidy = nSwapFaseReward;
    }

    if((randreward() <= 5000 || validation) && nHeight + 1 > 10080 && Params().NetworkID() == CBaseChainParams::MAIN) { // 5% Chance of superblock
       nSubsidy = nSuperPoWReward;
    }

    if((randreward() <= 5000 || validation) && nHeight + 1 > 10080 && Params().NetworkID() == CBaseChainParams::MAIN && (GetTime() > REWARD_MN_POW_SWITCH_TIME)) { // 5% Chance of superblock NEW
       nSubsidy = nSuperPoWReward_NEW;
    }
    // New reward version 3.0.0
    if ((nHeight + 1 > START_REWARD_3_0_T && Params().NetworkID() == CBaseChainParams::TESTNET) || (nHeight + 1 > START_REWARD_3_0_M && Params().NetworkID() == CBaseChainParams::MAIN)) {
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if((nHeight > START_REWARD_3_0_T && Params().NetworkID() == CBaseChainParams::TESTNET) ||
       (nHeight > START_REWARD_3_0_M && Params().NetworkID() == CBaseChainParams::MAIN)){
         ret = blockValue / 2; //50%
    }else{
       ret = 0.01 * COIN;
    }
...
bool CheckInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, bool fScriptChecks, unsigned int flags, bool cacheStore, std::vector<CScriptCheck>* pvChecks)
{
    if (!tx.IsCoinBase() && !tx.HasZerocoinSpendInputs()) {
		...
		//int nCoinbaseMaturity = coins->nHeight < 1000 ? 5 : Params().GetConsensus().nCoinbaseMaturity;
		//int nCoinbaseMaturity = coins->nHeight < consensus.height_last_PoW ? 5 : Params().GetConsensus().nCoinbaseMaturity;
		int nCoinbaseMaturity = (GetTime() <= (!(Params().NetworkID() == CBaseChainParams::TESTNET) ? NEW_MATURITY_SWITCH_TIME : NEW_MATURITY_SWITCH_TIME_TESTNET) ? Params().GetConsensus().nCoinbaseMaturity : Params().GetConsensus().nCoinbaseMaturity_NEW);
                //if (nSpendHeight - coins->nHeight < Params().GetConsensus().nCoinbaseMaturity)
                if (nSpendHeight - coins->nHeight < nCoinbaseMaturity && coins->nHeight >= consensus.height_last_PoW)
                    return state.Invalid(
                        error("CheckInputs() : tried to spend coinbase at depth %d, coinstake=%d, maturity=%d", nSpendHeight - coins->nHeight, coins->IsCoinStake(), nCoinbaseMaturity),
...
{
    util::ThreadRename("martex-scriptch");
...
*/
    if (!control.Wait() && pindex->pprev->nHeight > consensus.height_RHF)
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    // ----------- fastTX transaction scanning -----------
    if (sporkManager.IsSporkActive(SPORK_3_FASTSEND_BLOCK_FILTERING)) {
        for (const CTransaction& tx : block.vtx) {
            if (!tx.IsCoinBase()) {
...
std::string GetWarnings(std::string strFor)
{
    ...
    if (fLargeWorkForkFound) {
        ...
        strStatusBar = strRPC = _("Warning: Your blockchain doesn't match the required checkpoints! You have forked, please resync MarteX Core.");
    }
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
	...
	bool fMissingSporks = !pSporkDB->SporkExists(SPORK_17_COLDSTAKING_ENFORCEMENT);
        ...
        if (pfrom->cleanSubVer == "/MarteX Core:3.0.2.1/" || pfrom->cleanSubVer == "/MarteX Core:3.0.2.2/" || pfrom->cleanSubVer == "/MarteX Core:3.0.0.1/" ||
            pfrom->cleanSubVer == "/MarteX Core:3.0.0/" || pfrom->cleanSubVer == "/MarteX Core:3.0.3.1/" || pfrom->cleanSubVer == "/MarteX Core:3.0.3.2/" ||
            pfrom->cleanSubVer == "/MarteX Core:3.0.3.3/" || pfrom->cleanSubVer == "/MarteX Core:3.0.4/" || pfrom->cleanSubVer == "/MarteX Core:3.0.4.1/" ||
        ...
        if (strCommand == "tx") {
            ...
            if (pmn != NULL) {
                ...
                if (!mapAnonsendBroadcastTxes.count(tx.GetHash())) {
                    ...
                    mapAnonsendBroadcastTxes.insert(std::make_pair(tx.GetHash(), dstx));
                }
            }
        }
