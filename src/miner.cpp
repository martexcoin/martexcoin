...

    {
        ...
        for (std::map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                // zMXT spends can have (had) very large priority, use non-overflowing safe functions
...
void BitcoinMiner(CWallet* pwallet, bool fProofOfStake)
{
    ...
    util::ThreadRename("martex-miner");
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("MARTEXMiner exception");
    } catch (...) {
        LogPrintf("MARTEXMiner exception");
    }

    LogPrintf("MARTEXMiner exiting\n");
}
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    if ((chainActive.Tip()->nHeight + 1 > START_REWARD_3_0_T && Params().NetworkID() == CBaseChainParams::TESTNET) || (chainActive.Tip()->nHeight + 1 > START_REWARD_3_0_M && Params().NetworkIDString() == CBaseChainParams::MAIN)) {
