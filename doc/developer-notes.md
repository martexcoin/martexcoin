...
MARTEX Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
([`valgrind.supp`](https://github.com/martex-project/martex/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_martex
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_martex --log_level=test_suite
$ valgrind -v --leak-check=full src/martexd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_martex.coverage/index.html`.
```
...
- BitcoinMiner : Generates MXTs (if wallet is enabled).
...
file descriptors it will cause problems with MARTEX's `select()` loop, because
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `martex-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `martex-cli` and the GUI debug console use this table to determine how to
