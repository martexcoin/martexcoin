...
The build script we'll be using is contained in the MARTEX github repository ([contrib/gitian-build.py](https://github.com/martex-project/martex/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the MARTEX repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/MARTEX-Project/MARTEX/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, martex-detached-sigs, and martex GitHub repos.
